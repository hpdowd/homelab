...
| k8s/nextcloud-cron-multiattach-rwo.md | k8s | `nextcloud-cron` failed ~half its runs with `KubeJobFailed` — RWO `nextcloud-data` attaches to the main pod's node, but the CronJob had no affinity so off-node runs hit `FailedAttachVolume` → `DeadlineExceeded`. HOMELAB.md already documented "podAffinity pins to worker"; the manifest had drifted. Fixed with podAffinity on `app: nextcloud`. `nextcloud-backup` still has the same latent gap |
