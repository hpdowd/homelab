...
| k8s/worker-reboot-alert-storm.md | k8s | A worker reboot fired ~20 `PodCrashLooping` alerts ~20 min late, all for already-healthy pods. Longhorn reattach crashloops during boot + the alerter being on the same worker (down, then batch-flushed on recovery). Benign; triage by the *absence* of `PodOOMKilled` |
