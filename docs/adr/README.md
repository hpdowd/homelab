...
| 017-bootstrap-layer.md | `bootstrap/` as a third layer between `ansible/` and `k8s/`: a pinned, idempotent shell script replacing ten hand-copied runbook steps that installed from `latest`/`stable`/`main` and described a cluster shaped differently from this one. Ansible rejected (converges state, wrong shape for a once-run ordered sequence), Terraform rejected (state file with nowhere durable to live), ArgoCD impossible (it installs ArgoCD). Sealed Secrets declared for the first time, manual-sync like longhorn.yaml; its chart version is knowingly pinned twice |
| 018-authelia-sso.md | Authelia 4.39.20 as the SSO layer: file users backend + SQLite, ForwardAuth for browser-only apps (kiwix/amp/homepage/proxmox) and OIDC for anything with a non-browser client (immich/nextcloud/gitea/paperless). Authentik rejected on RAM, Cloudflare-Access-for-everything rejected because it can't cover the LAN and makes Cloudflare a hard dependency. The LAN is deliberately left outside the perimeter so a broken Authelia can't lock me out of the hypervisor that hosts it; SMTP and NTP startup checks declawed so a mail or time outage can't become an auth outage; DB treated as regenerable, so no backup and no `Prune=false`. `home.dowd.ie` was deferred, then **resolved 2026-09-03 by dropping the host** — it served the same pod on a second apex, so gating `dash.henrydowd.dev` alone left the dashboard world-readable; the homepage widgets are now unblocked |
