...
| 018-authelia-sso.md | Authelia 4.39.20 as the SSO layer: file users backend + SQLite, ForwardAuth for browser-only apps (kiwix/amp/homepage/proxmox) and OIDC for anything with a non-browser client (immich/nextcloud/gitea/paperless). Authentik rejected on RAM, Cloudflare-Access-for-everything rejected because it can't cover the LAN and makes Cloudflare a hard dependency. The LAN is deliberately left outside the perimeter so a broken Authelia can't lock me out of the hypervisor that hosts it; SMTP and NTP startup checks declawed so a mail or time outage can't become an auth outage; DB treated as regenerable, so no backup and no `Prune=false`. `home.dowd.ie` deferred, and the homepage widgets stay blocked because of it |
